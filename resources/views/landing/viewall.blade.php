    ...
    <div class="container mx-auto py-8 mt-20">
        ...
        <div class="mb-4 flex justify-center">
            <form action="{{ route('allplace') }}" method="GET" class="flex w-1/2">
                <input type="text" name="search" placeholder="Search..." value="{{ $search ?? '' }}" class="border border-gray-300 px-4 py-2 rounded-full w-full mr-2">
                <select name="bidang" class="border border-gray-300 px-4 py-2 rounded-full mr-2">
                    <option value="">All Fields</option>
                    @foreach ($bidangs as $bidangItem)
                        <option value="{{ $bidangItem->bidang }}" {{ $bidang == $bidangItem->bidang ? 'selected' : '' }}>{{ $bidangItem->bidang }}</option>
                    @endforeach
                </select>
        ...
        <div class="mt-8 flex justify-center">
            @if ($perusahaan->hasPages())
                ...
                @else
                    <a href="{{ $perusahaan->previousPageUrl() }}" class="px-3 py-1 bg-blue-500 text-white rounded-md">Previous</a>
                @endif
                ...
                @foreach ($perusahaan->getUrlRange(1, $perusahaan->lastPage()) as $page => $url)
                    ...
                    @else
                        <a href="{{ $url }}" class="px-3 py-1 bg-blue-500 text-white rounded-md ml-2">{{ $page }}</a>
                    @endif
                @endforeach

                {{-- Next Page Link --}}
                @if ($perusahaan->hasMorePages())
                    <a href="{{ $perusahaan->nextPageUrl() }}" class="px-3 py-1 bg-blue-500 text-white rounded-md ml-2">Next</a>
                @else
